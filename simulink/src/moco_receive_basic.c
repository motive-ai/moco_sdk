...
#define S_FUNCTION_NAME  moco_receive_basic
#define S_FUNCTION_LEVEL 2
#define NPARAMS          0

#define OUT_PORT_0_NAME current_q
#define OUT_PORT_1_NAME mode
#define OUT_PORT_2_NAME motor_enc
#define IN_PORT_0_NAME moco_ctx

#include "simstruc.h"
#include "moco_receive_basic.h"
...
//see: https://www.mathworks.com/help/simulink/ug/control-linked-block-status-programmatically.html
    ...
    if (dim_array[1] < 5) { //not linked
        ...
        normInputs[2] = mxCreateString("port_label('output', 1, 'current_q');port_label('output', 2, 'mode');port_label('output', 3, 'motor_enc');port_label('input', 1, 'moco_ctx');disp('moco_receive_basic')");
        mexCallMATLAB(0, 0, 3, normInputs, "set_param");
