...
#define S_FUNCTION_NAME  moco_receive_sin_position_command
#define S_FUNCTION_LEVEL 2
#define NPARAMS          0

#define OUT_PORT_0_NAME amplitude
#define OUT_PORT_1_NAME bias
#define OUT_PORT_2_NAME frequency
#define IN_PORT_0_NAME moco_ctx

#include "simstruc.h"
#include "moco_receive_sin_position_command.h"
...
//see: https://www.mathworks.com/help/simulink/ug/control-linked-block-status-programmatically.html
    ...
    if (dim_array[1] < 5) { //not linked
        ...
        normInputs[2] = mxCreateString("port_label('output', 1, 'amplitude');port_label('output', 2, 'bias');port_label('output', 3, 'frequency');port_label('input', 1, 'moco_ctx');disp('moco_receive_sin_position_command')");
        mexCallMATLAB(0, 0, 3, normInputs, "set_param");
...
static void mdlOutputs(SimStruct *S, int_T tid) {
    moco_receive_sin_position_command_outputs(S);
}
