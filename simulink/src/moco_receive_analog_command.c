...
#define S_FUNCTION_NAME  moco_receive_analog_command
#define S_FUNCTION_LEVEL 2
#define NPARAMS          0

#define OUT_PORT_0_NAME analog_desired
#define OUT_PORT_1_NAME motor_torque_feed_forward
#define IN_PORT_0_NAME moco_ctx

#include "simstruc.h"
#include "moco_receive_analog_command.h"
...
//see: https://www.mathworks.com/help/simulink/ug/control-linked-block-status-programmatically.html
    ...
    if (dim_array[1] < 5) { //not linked
        ...
        normInputs[2] = mxCreateString("port_label('output', 1, 'analog_desired');port_label('output', 2, 'motor_torque_feed_forward');port_label('input', 1, 'moco_ctx');disp('moco_receive_analog_command')");
        mexCallMATLAB(0, 0, 3, normInputs, "set_param");
    ...
    ssSetNumRWork(         S, 2);   // number of real work vector elements
...
static void mdlOutputs(SimStruct *S, int_T tid) {
    moco_receive_analog_command_outputs(S);
}
