...
#define S_FUNCTION_NAME  moco_command_position
#define S_FUNCTION_LEVEL 2
#define NPARAMS          0

#define IN_PORT_0_NAME moco_ctx
#define IN_PORT_1_NAME motor_torque_feed_forward
#define IN_PORT_2_NAME position
#define IN_PORT_3_NAME velocity

#include "simstruc.h"
#include "moco_command_position.h"
...
//see: https://www.mathworks.com/help/simulink/ug/control-linked-block-status-programmatically.html
    ...
    if (dim_array[1] < 5) { //not linked
        ...
        normInputs[2] = mxCreateString("port_label('input', 1, 'moco_ctx');port_label('input', 2, 'motor_torque_feed_forward');port_label('input', 3, 'position');port_label('input', 4, 'velocity');disp('moco_command_position')");
        mexCallMATLAB(0, 0, 3, normInputs, "set_param");
...
static void mdlOutputs(SimStruct *S, int_T tid) {
    moco_command_position_outputs(S);
}
