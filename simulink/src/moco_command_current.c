...
#define S_FUNCTION_NAME  moco_command_current
...
#include "simstruc.h"
#include "moco_command_current.h"
...
//see: https://www.mathworks.com/help/simulink/ug/control-linked-block-status-programmatically.html
    ...
    if (dim_array[1] < 5) { //not linked
        ...
        normInputs[2] = mxCreateString("port_label('input', 1, 'moco_ctx');port_label('input', 2, 'i_d');port_label('input', 3, 'i_q');disp('moco_command_current')");
        mexCallMATLAB(0, 0, 3, normInputs, "set_param");
...
static void mdlOutputs(SimStruct *S, int_T tid) {
    moco_command_current_outputs(S);
}
