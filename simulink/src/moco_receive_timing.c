...
#define S_FUNCTION_NAME  moco_receive_timing
#define S_FUNCTION_LEVEL 2
#define NPARAMS          0

#define OUT_PORT_0_NAME cla_main.execution
#define OUT_PORT_1_NAME cla_main.period
#define OUT_PORT_2_NAME controller_main.execution
#define OUT_PORT_3_NAME controller_main.period
#define OUT_PORT_4_NAME cpu_main.execution
#define OUT_PORT_5_NAME cpu_main.period
#define OUT_PORT_6_NAME usb_interrupt.execution
#define OUT_PORT_7_NAME usb_interrupt.period
#define IN_PORT_0_NAME moco_ctx

#include "simstruc.h"
#include "moco_receive_timing.h"

static void mdlInitializeSizes(SimStruct *S) {
    int_T nInputPorts  = 1;  // number of input ports
    int_T nOutputPorts = 8;  // number of output ports
...
//see: https://www.mathworks.com/help/simulink/ug/control-linked-block-status-programmatically.html
    ...
    if (dim_array[1] < 5) { //not linked
        ...
        normInputs[2] = mxCreateString("port_label('output', 1, 'cla_main.execution');port_label('output', 2, 'cla_main.period');port_label('output', 3, 'controller_main.execution');port_label('output', 4, 'controller_main.period');port_label('output', 5, 'cpu_main.execution');port_label('output', 6, 'cpu_main.period');port_label('output', 7, 'usb_interrupt.execution');port_label('output', 8, 'usb_interrupt.period');port_label('input', 1, 'moco_ctx');disp('moco_receive_timing')");
        mexCallMATLAB(0, 0, 3, normInputs, "set_param");
    ...
    ssSetNumIWork(         S, 8);   // number of integer work vector elements
