...
#define S_FUNCTION_NAME  moco_receive_current_command
...
#include "simstruc.h"
#include "moco_receive_current_command.h"
...
//see: https://www.mathworks.com/help/simulink/ug/control-linked-block-status-programmatically.html
    ...
    if (dim_array[1] < 5) { //not linked
        ...
        normInputs[2] = mxCreateString("port_label('output', 1, 'i_d');port_label('output', 2, 'i_q');port_label('input', 1, 'moco_ctx');disp('moco_receive_current_command')");
        mexCallMATLAB(0, 0, 3, normInputs, "set_param");
...
static void mdlOutputs(SimStruct *S, int_T tid) {
    moco_receive_current_command_outputs(S);
}
