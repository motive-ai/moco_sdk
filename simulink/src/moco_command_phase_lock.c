...
#define S_FUNCTION_NAME  moco_command_phase_lock
#define S_FUNCTION_LEVEL 2
#define NPARAMS          0

#define IN_PORT_0_NAME moco_ctx
#define IN_PORT_1_NAME i_d
#define IN_PORT_2_NAME i_q
#define IN_PORT_3_NAME phase_angle

#include "simstruc.h"
#include "moco_command_phase_lock.h"
...
//see: https://www.mathworks.com/help/simulink/ug/control-linked-block-status-programmatically.html
    ...
    if (dim_array[1] < 5) { //not linked
        ...
        normInputs[2] = mxCreateString("port_label('input', 1, 'moco_ctx');port_label('input', 2, 'i_d');port_label('input', 3, 'i_q');port_label('input', 4, 'phase_angle');disp('moco_command_phase_lock')");
        mexCallMATLAB(0, 0, 3, normInputs, "set_param");
...
static void mdlOutputs(SimStruct *S, int_T tid) {
    moco_command_phase_lock_outputs(S);
}
