...
#define S_FUNCTION_NAME  moco_mode_phase_lock
#define S_FUNCTION_LEVEL 2
#define NPARAMS          0

#define IN_PORT_0_NAME moco_ctx
#define IN_PORT_1_NAME current_param.max_current
#define IN_PORT_2_NAME current_param.pi_d.ki
#define IN_PORT_3_NAME current_param.pi_d.ki_sat
#define IN_PORT_4_NAME current_param.pi_d.kp
#define IN_PORT_5_NAME current_param.pi_q.ki
#define IN_PORT_6_NAME current_param.pi_q.ki_sat
#define IN_PORT_7_NAME current_param.pi_q.kp
#define IN_PORT_8_NAME phase_lock_current

#include "simstruc.h"
#include "moco_mode_phase_lock.h"

static void mdlInitializeSizes(SimStruct *S) {
    int_T nInputPorts  = 9;  // number of input ports
...
//see: https://www.mathworks.com/help/simulink/ug/control-linked-block-status-programmatically.html
    ...
    if (dim_array[1] < 5) { //not linked
        ...
        normInputs[2] = mxCreateString("port_label('input', 1, 'moco_ctx');port_label('input', 2, 'current_param.max_current');port_label('input', 3, 'current_param.pi_d.ki');port_label('input', 4, 'current_param.pi_d.ki_sat');port_label('input', 5, 'current_param.pi_d.kp');port_label('input', 6, 'current_param.pi_q.ki');port_label('input', 7, 'current_param.pi_q.ki_sat');port_label('input', 8, 'current_param.pi_q.kp');port_label('input', 9, 'phase_lock_current');disp('moco_mode_phase_lock')");
        mexCallMATLAB(0, 0, 3, normInputs, "set_param");
...
static void mdlOutputs(SimStruct *S, int_T tid) {
    moco_mode_phase_lock_outputs(S);
}
