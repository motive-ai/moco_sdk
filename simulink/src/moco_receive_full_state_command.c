...
#define S_FUNCTION_NAME  moco_receive_full_state_command
#define S_FUNCTION_LEVEL 2
#define NPARAMS          0

#define OUT_PORT_0_NAME current_feed_forward
#define OUT_PORT_1_NAME kd
#define OUT_PORT_2_NAME kp
#define OUT_PORT_3_NAME ks
#define OUT_PORT_4_NAME kt
#define OUT_PORT_5_NAME position
#define OUT_PORT_6_NAME torque
#define OUT_PORT_7_NAME torque_dot
#define OUT_PORT_8_NAME velocity
#define IN_PORT_0_NAME moco_ctx

#include "simstruc.h"
#include "moco_receive_full_state_command.h"

static void mdlInitializeSizes(SimStruct *S) {
    int_T nInputPorts  = 1;  // number of input ports
    int_T nOutputPorts = 9;  // number of output ports
...
//see: https://www.mathworks.com/help/simulink/ug/control-linked-block-status-programmatically.html
    ...
    if (dim_array[1] < 5) { //not linked
        ...
        normInputs[2] = mxCreateString("port_label('output', 1, 'current_feed_forward');port_label('output', 2, 'kd');port_label('output', 3, 'kp');port_label('output', 4, 'ks');port_label('output', 5, 'kt');port_label('output', 6, 'position');port_label('output', 7, 'torque');port_label('output', 8, 'torque_dot');port_label('output', 9, 'velocity');port_label('input', 1, 'moco_ctx');disp('moco_receive_full_state_command')");
        mexCallMATLAB(0, 0, 3, normInputs, "set_param");
...
static void mdlOutputs(SimStruct *S, int_T tid) {
    moco_receive_full_state_command_outputs(S);
}
