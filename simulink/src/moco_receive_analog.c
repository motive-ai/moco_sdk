...
#define S_FUNCTION_NAME  moco_receive_analog
#define S_FUNCTION_LEVEL 2
#define NPARAMS          0

#define OUT_PORT_0_NAME analog_a
#define OUT_PORT_1_NAME analog_b
#define OUT_PORT_2_NAME analog_diff
#define IN_PORT_0_NAME moco_ctx

#include "simstruc.h"
#include "moco_receive_analog.h"
...
//see: https://www.mathworks.com/help/simulink/ug/control-linked-block-status-programmatically.html
    ...
    if (dim_array[1] < 5) { //not linked
        ...
        normInputs[2] = mxCreateString("port_label('output', 1, 'analog_a');port_label('output', 2, 'analog_b');port_label('output', 3, 'analog_diff');port_label('input', 1, 'moco_ctx');disp('moco_receive_analog')");
        mexCallMATLAB(0, 0, 3, normInputs, "set_param");
