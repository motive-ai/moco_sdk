...
#define S_FUNCTION_NAME  moco_receive_spi_accelerometer
...
#include "simstruc.h"
#include "moco_receive_spi_accelerometer.h"
...
//see: https://www.mathworks.com/help/simulink/ug/control-linked-block-status-programmatically.html
    ...
    if (dim_array[1] < 5) { //not linked
        ...
        normInputs[2] = mxCreateString("port_label('output', 1, 'x');port_label('output', 2, 'y');port_label('output', 3, 'z');port_label('input', 1, 'moco_ctx');disp('moco_receive_spi_accelerometer')");
        mexCallMATLAB(0, 0, 3, normInputs, "set_param");
...
static void mdlOutputs(SimStruct *S, int_T tid) {
    moco_receive_spi_accelerometer_outputs(S);
}
