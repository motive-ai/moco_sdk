...
#define S_FUNCTION_NAME  moco_receive_torque
#define S_FUNCTION_LEVEL 2
#define NPARAMS          0

#define OUT_PORT_0_NAME ch1
#define OUT_PORT_1_NAME ch2
#define OUT_PORT_2_NAME error_count
#define OUT_PORT_3_NAME period
#define OUT_PORT_4_NAME torque
#define IN_PORT_0_NAME moco_ctx

#include "simstruc.h"
#include "moco_receive_torque.h"

static void mdlInitializeSizes(SimStruct *S) {
    int_T nInputPorts  = 1;  // number of input ports
    int_T nOutputPorts = 5;  // number of output ports
...
//see: https://www.mathworks.com/help/simulink/ug/control-linked-block-status-programmatically.html
    ...
    if (dim_array[1] < 5) { //not linked
        ...
        normInputs[2] = mxCreateString("port_label('output', 1, 'ch1');port_label('output', 2, 'ch2');port_label('output', 3, 'error_count');port_label('output', 4, 'period');port_label('output', 5, 'torque');port_label('input', 1, 'moco_ctx');disp('moco_receive_torque')");
        mexCallMATLAB(0, 0, 3, normInputs, "set_param");
    ...
    ssSetNumIWork(         S, 4);   // number of integer work vector elements
